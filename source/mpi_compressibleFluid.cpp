...
namespace Fluid
{
    ...
    const Point<dim> &p, const unsigned int component) const
  {
    ...
    if (component == 0 && std::abs(p[0]) < 1e-8)
      {
        return 6.0 * exp(-0.5 * pow((time - 0.5e-4) / 0.15e-4, 2));
      }
    return 0;
  }
    ...
    // desired, this block of code has to be modified.
    {
      ...
      for (auto itr = parameters.fluid_dirichlet_bcs.begin();
           ...
           ++itr)
        {
          ...
          apply_nonzero_boundary_values(id);
  ...
  template <int dim>
  void ParallelCompressibleFluid<dim>::apply_nonzero_boundary_values(const unsigned int id)
  {
    TimeDependentBoundaryValues bc_function(time.current());
    //BoundaryValues bc_function;
         ...
         ++cell)
      {
        ...
        for (unsigned int face_no = 0;
             ...
             ++face_no)
          {
            ...
            if (boundary_component == id && (cell->get_fe().dofs_per_face > 0))
              {
                std::vector<unsigned int> v_list(
                  GeometryInfo<dim>::vertices_per_face);
                for (unsigned int v = 0; v < v_list.size(); ++v)
                  {
                    v_list[v] =
                      GeometryInfo<dim>::face_to_cell_vertices(face_no, v);
                    for (unsigned int i = 0; i < dim; ++i)
                      {
                        auto line = cell->vertex_dof_index(v, i);
                        nonzero_constraints.add_line(line);
                        // Note that we are setting the value of the constraint
                        // to the
                        // velocity delta!
                        Point<dim> point = cell->vertex(v);
                        double bc_value = bc_function.value(point, i);
                        if (time.current() > 1e-12)
                          nonzero_constraints.set_inhomogeneity(
                            line, bc_value - present_solution(line));
                        else nonzero_constraints.set_inhomogeneity(
                            line, bc_value);
                      }
                  }
              }
    ...
    template <int dim>
    void ParallelCompressibleFluid<dim>::assemble(
      const bool use_nonzero_constraints)
    {
    ...
    template <int dim>
    std::pair<unsigned int, double> ParallelCompressibleFluid<dim>::solve(
      const bool use_nonzero_constraints)
    {
    ...
    template <int dim>
    void ParallelCompressibleFluid<dim>::refine_mesh(
    ...
    template <int dim>
    void ParallelCompressibleFluid<dim>::run_one_step(
      bool apply_nonzero_constraints)
    {
